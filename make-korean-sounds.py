...
TTS_VOICE = os.getenv("TTS_VOICE", "coral")   # 따뜻하고 친근한 목소리
# (cedar/marin 은 표현력이 좋지만 조각마다 편차가 커서 이어 붙이면 튑니다.
#  coral 은 다정하면서도 비교적 균일해 안내에 잘 맞습니다. 담담한 쪽은 alloy/nova.
#  다른 목소리는 --voice 로. shimmer(더 부드러움)도 친근한 편.)
...
#  강한 지시로 최대한 줄이는 방식입니다. 그래도 튀는 조각은 그 조각만 다시 생성.)
TTS_INSTRUCTIONS = os.getenv(
    "TTS_INSTRUCTIONS",
    "한국어 전화 안내(ARS) 녹음입니다. "
    "이 음성은 여러 조각으로 따로 녹음한 뒤 그대로 '이어 붙여' 한 문장으로 "
    "재생됩니다. 그러므로 조각마다 목소리가 달라지면 안 됩니다. "
    "가장 중요한 것: 매 조각을 '완전히 똑같이' — 같은 목소리, 같은 피치, 같은 속도, "
    "같은 크기, 같은 다정한 톤으로 — 한 사람이 한 번에 녹음한 것처럼 균일하게 읽어 주세요. "
    "톤: 친절하고 따뜻한 안내원 목소리. 살짝 미소 띤 듯 다정하되, 과장하거나 들뜨지 말고 "
    "차분하게. 그 다정한 느낌을 조각마다 똑같이 유지해서 감정 기복이 생기지 않게 하세요. "
    "피치: 중간 높이로 처음부터 끝까지 일정하게. 문장 끝에서 질문하듯 올리거나 "
    "마침표처럼 뚝 내리지 말고, 시작과 같은 높이로 담담하게 끝내세요 "
    "(그래야 다음 조각과 매끄럽게 이어집니다). "
    "속도: 평소의 약 90퍼센트로 느리고 일정하게. 처음부터 끝까지 같은 속도이고, "
    "문장 끝을 흐리거나 빨라지지 않게. "
    "발음: 전화 스피커는 음질이 좁고 어르신도 들으시니 자음을 특히 분명하게, "
    "웅얼거리지 말고 또박또박. 숫자·시각·요일은 한 음절씩 또렷하게 끊어 읽고, "
    "'공'은 짧고 분명하게, '일곱 시 반' 같은 표현은 서두르지 말고 천천히. "
    "호흡: 숨소리·한숨·군말·잡음 없이 깔끔하게. 쉼표에서는 반 박자 쉬었다가 이어 주세요. "
    "다시 강조합니다 — 모든 조각이 하나의 균일한 목소리로 들려야 합니다.")
...
GREETINGS = {
    ...
    # "몇 분" 은 표준 발음이 [멷뿐] 이라 또박또박 읽어도 붙어 들립니다.
    # 메뉴에서는 아예 '몇 분' 을 빼고 "잠시 후" 로 말합니다(입력 단계에서 분을 받습니다).
    "g-menu-1":    "잠시 후 알림 받으시려면 일번,",
    "g-menu-2":    "시간을 정해 두시려면 이번,",
    "g-menu-3":    "반복 알람을 맞추시려면 삼번,",
    "g-menu-4":    "예약 목록을 확인하시려면 사번,",
    "g-menu-5":    "알람 소리를 바꾸시려면 오번을 눌러 주세요.",
    ...
    # 여는 말도 '몇 분' 을 피해 "언제" 로 물어봅니다(발음이 뭉치지 않습니다).
    "g-enter-min": "알람을 언제 맞춰 드릴까요? "
                   "분 단위로 숫자만 눌러 주세요. "
                   "삼십 분 후면 삼공입니다.",
    ...
    "g-today":     "오늘",
    "g-tomorrow":  "내일",
    # 예약 시각에 걸려오는 전화는 멘트 없이 알림음만 울립니다(음성 조각 안 씀).
}
...
def targets() -> list[tuple[Path, str]]:
    ...
    # 첫 소리 앞에 이 무음을 깔아 첫 음절이 잘리는 걸 막습니다.
...
def _norm_pass(input_args: list[str], rate: int, dst: Path) -> None:
    """무음 정리 (+클리어 필터) + 리샘플 → 음량 측정 → 고정 이득 적용 (2단계).

    필터를 음량 측정 '전에' 걸어야 정규화가 필터 후 레벨을 기준으로 잡습니다.
    """
    pre = Path(str(dst) + ".pre.wav")
    af = SILENCE_TRIM + ("," + CLARITY_FILTER if CLARITY else "")
    subprocess.run(["ffmpeg", "-y", "-loglevel", "error", *input_args,
                    "-ac", "1", "-af", af, "-ar", str(rate),
