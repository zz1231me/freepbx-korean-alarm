...
#   2) 한국어 안내 음성 조각 161개 생성  <- 여기서만 인터넷/TTS 를 씁니다
...
#------------------------------------------------------------------------------
log "2/4  한국어 안내 음성 만들기 (161개, 몇 분 걸립니다)"
#------------------------------------------------------------------------------
...
echo "  조각 ${N}/161 개 (8kHz) + ${N16}/161 개 (16kHz)"
(( N16 >= 161 )) || warn "16kHz 판이 부족합니다. G.722 통화에서 소리가 뭉개질 수 있습니다"
(( N >= 161 )) || warn "조각이 부족합니다. 부족한 시각은 안내가 조용할 수 있습니다"
...
===========================================================================
 ...
 사용  —  7000 하나만 기억하시면 됩니다
   7000 -> "시간 알림 써비스입니다."
        -> (한 항목씩 화면 넘기며) "잠시 후 알림 받으시려면 일번,
            시간을 정해 두시려면 이번, 반복 알람을 맞추시려면 삼번,
            예약 목록을 확인하시려면 사번."

     1번  30          -> "삼십 분 후 알람 설정하였습니다"
     2번  1900        -> "오늘 저녁 일곱시 정각 알람 설정하였습니다"
     3번  1 → 0730    -> "평일 오전 일곱시 반 알람 설정하였습니다.
                          내일부터 시작합니다"
     4번             -> "예약된 알람입니다." (한 건씩)
                          "매일 오전 일곱 시. 수정 일번, 삭제 이번, 다음은 잠시 기다림."
                           1=수정(지우고 다시), 2=삭제, 무입력=다음, *=끝
 ...
 확인
   ...
   ls /var/lib/asterisk/sounds/custom/bank/*.wav | wc -l   # 161 이어야 합니다
   ls /var/lib/asterisk/sounds/custom/bank/*.wav16 | wc -l # 161 (16kHz 판)
